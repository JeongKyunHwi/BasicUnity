...
public class HitLazer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dir = Pos - tr.position; // 마우스 - 플레이어 포지션(마우스 방향으로가는 벡터)
    ...
    {
        //회전 적용
        //사원수(Quaternion)를 써야 하는 이유는 크게 깜빡임(Gimbal Lock) 방지, 부드러운 회전(Interpolation),
        //안정적인 연산이라는 세 가지 핵심 이유가 있어.

        // 1.깜빡임(Gimbal Lock) 방지
        //깜빡임(Gimbal Lock)은 오일러 각(Euler Angles)로 회전을 표현할 때 특정한 각도에서 축 하나가 잠겨버려서
        //회전이 제대로 안 되는 문제야.
